...
export const typography = (width: number): Record<string, TextStyle> => {
  return {
    ...
    // Main content
    body: {
      fontSize: width * 0.040,       // ~15px
      fontWeight: '400',
      lineHeight: width * 0.060,     // 1.5x for readability
      letterSpacing: 0.01 * width * 0.040,
    },
    // Emphasized body
    bodyBold: {
    ...
    // Fine print
    small: {
      fontSize: width * 0.030,       // ~11px
      fontWeight: '400',
      lineHeight: width * 0.042,
      letterSpacing: 0.02 * width * 0.030,
    },
  };
};
...
export const borderRadius = (width: number) => {
  return {
    ...
    md: width * 0.020,   // ~8px - standard buttons
 ...
 */
export const getDeviceSize = (width: number): 'small' | 'medium' | 'large' | 'xlarge' => {
